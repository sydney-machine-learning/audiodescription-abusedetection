...
# Functions used to manage interim files created in STT pipeline and subtitle cleaning
...
all_transcripts_df_dir = os.path.join(transcript_dir, 'all_transcripts.parquet')
diarization_dir = os.path.join(interim_dir, 'diarization_segments')
...
cleaned_dataset_fp = os.path.join(processed_dir, 'cleaned_dataset.parquet')
...
def get_transcript_list():
    return [x for x in os.listdir(transcript_dir) if x.endswith(transcript_df_fp.format(movie_name=''))]
...
def get_sorted_mp3_list(rerun_all: bool = False):
    ...
    if rerun_all:
        ...
        file_times = [(movie + '.mp3', os.stat(os.path.join(transcript_dir, transcript_df_fp.format(movie_name=movie))).st_mtime) for movie in filtered_transcripts]
...
def wipe_movie_files(movie_name: str):
    ...
    if os.path.exists(os.path.join(transcript_dir, transcript_df_fp.format(movie_name=movie_name))):
        os.remove(os.path.join(transcript_dir, transcript_df_fp.format(movie_name=movie_name)))
...
def _rename_specific_file(old_fp: str, new_fp: str, new_name: str):
...
def rename_movie_files(movie_name: str, new_name: str):
    ...
    old_trans_fp = os.path.join(transcript_dir, transcript_df_fp.format(movie_name=movie_name))
    ...
    if os.path.exists(old_trans_fp):
        new_trans_fp = os.path.join(transcript_dir, transcript_df_fp.format(movie_name=new_name))
        _rename_specific_file(old_trans_fp, new_trans_fp, new_name)
        rename_nothing = False
        
    if os.path.exists(old_diaz_fp):
        new_diaz_fp = os.path.join(diarization_dir, f'{new_name}-diarization.parquet')
        _rename_specific_file(old_diaz_fp, new_diaz_fp, new_name)
        rename_nothing = False
        
    if os.path.exists(old_vad_fp):
        new_vad_fp = os.path.join(voice_activity_dir, f'{new_name}-vad.parquet')
        _rename_specific_file(old_vad_fp, new_vad_fp, new_name)
        rename_nothing = False
